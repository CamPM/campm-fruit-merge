...
import { CosmeticState, GameState, PhysicsProps } from './types.ts';
import { PHYSICS_HIERARCHY, GAME_WIDTH, GAME_HEIGHT, EMOJI_SETS, TIER_COLORS, BORDERS } from './constants.ts';
  ...
  const render = useCallback(() => {
    ...
    bodies.forEach(body => {
      ...
      if (gameState.activePowerUp === 'bomb') {
        // Fix: Removed redundant 'dist' calculation using undefined 'x' and 'y'
        const tx = touchPosRef.current.x;
        const ty = touchPosRef.current.y;
        const d = Math.sqrt(Math.pow(body.position.x - tx, 2) + Math.pow(body.position.y - ty, 2));
        if (d < 80 + getRadius(body.tier)) isTargeted = true;
